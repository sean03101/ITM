...
class Field():
 ...
 It's okay for (row, col) to be outside the field."""
  ...
  # (row, col) has 0 bombs in its neighborhood,
  # uncover the entire safe region
  def _safe_region(self, row, col):
    for i in range(-1, 2):
      for j in range(-1, 2):
        if (0 <= row + i < self._nrows and
            0 <= col + j < self._ncols and
            self.cell(row + i, col + j) == '.'):
