...
통계 검정의 결과로서 자유도 16, 유의확률 0.220 &rarr; 일반적인 신뢰수준에서 유의하지 않음



### 시뮬레이션 가정

- 2가지 경우에 대해 몬테카를로 시뮬레이션 연구 수행
    1. 감염률이 0.0027%일 경우(2020년 3월 15일 서울시 인구수 대비 확진자 수)
    2. 감염률이 0.01%일 경우(돌연변이로 인한 감염률 상승의 경우)

> 시뮬레이션 진행 과정 예시

<p align="center">
 <img src = "./img/시뮬 과정 요약.png">
</p>
<br>


Step 1) (하루에 서울에서 부산으로 이동하는 인구 * 감염률 * 최초 확진자 비율)을 모수로 하는 포아송 분포 생성
<br>
Step 2) 포아송 분포를 통해 부산으로 이동하는 가상 이동 최초 확진자 난수 생성 
<br>
Step 3) 추정한 주간 전파율 분포를 통해 1주일간 부산에서 감염된 확진자 수 추정



### 시뮬레이션 결과

> 10,000번의 시뮬레이션 수행 결과(감염률이 0.0027%인 경우)
...
> 10,000번의 시뮬레이션 수행 결과(감염률이 0.01%인 경우)
...
- 평균적으로 생성되는 피감염자의 수는 이동하는 인원의 수와 서울시의 감염률에 대체로 비례
- 2020년의 대폭 줄어든 도시간 이동량은, 2019년의 두 번째 행에 해당하는 상대적으로 우려할만한 대규모 도시간 이동으로 인한 감염을 방지하는 효과가 있음
- 감염 분포는 Skewed 분포이기에 최악의 경우가 더욱 극단적으로 발생할 가능성이 충분히 존재
- 철도 이동량이 적은 경우라고 하더라도 수십명의 감염이 발생할 수 있음(like 이태원 클럽발 확진 사태)

### 결론

- 질병과 전쟁과 같은 특수 위험 상황에 관한 시뮬레이션 연구에서는 비관적인 가정하게 연구를 수행
- 특별한 이벤트가 없는 3월 1주일을 기준으로 하여 코로나 발발 전 교통 이동량에 따른 질병 전파 시뮬레이션 진행
- 하지만 실제 상황에서는 다음과 같은 상황때문에 더 많은 피감염자 발생가능성 존재
    1. 1주일 이상의 오랜 기간을 설정
    2. 상대적으로 낮은 확진율로 인해서 서울 외 지역에서 경각심이 낮을 수 있음
    3. 부산뿐만 아니라 대구, 대전 등 여러 대도시에서 동시다발적으로 확진자가 이동
- 본 실험 및 다른 나라에서의 상황을 고려하자면 지역 간 이동으로 인해 수많은 확진자가 나올 가능성이 다분